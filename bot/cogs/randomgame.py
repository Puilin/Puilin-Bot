...
class RandomGame(commands.Cog):
    ...
    @app_commands.command(name="가위바위보", description="퓨이린 봇과 가위바위보를 합니다.")
    async def rsp(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if res.content == '/시작':
                await ctx.send("가위, 바위, 보 중에 고르세요! 제한시간은 10초입니다!")
                def check2(res2):
                ...
                try:
                    ...
                    if res2.content == '가위':
                        if bots_choice[0] == '가위':
                            await ctx.send("비겼습니다. 봇의 선택 : %s" %(bots_choice[0]))
                        elif bots_choice[0] == '바위':
                            await ctx.send("졌습니다... 봇의 선택 : %s" %(bots_choice[0]))
                        else:
                            await ctx.send("이겼습니다! 봇의 선택 : %s" %(bots_choice[0]))
                    ...
                    await ctx.send("10초가 지났습니다. 당신의 패배입니다.")
            else:
                await ctx.send("게임이 종료되었습니다.")
        except asyncio.TimeoutError:
            await ctx.send("게임이 취소되었습니다. 장비를 정지합니다.")

    @app_commands.command(name="끝말잇기", description="퓨이린 봇과 끝말잇기를 합니다.")
    async def kkut(self, interaction: discord.Interaction):
        ctx = await commands.Context.from_interaction(interaction)
        ...
        try:
            ...
            if res.content == '/시작':
                await ctx.send("봇이 먼저 시작합니다! 제한 시간은 5초입니다!")
                history = []
                ...
                bots_word = list2
                await ctx.send("%s" %random.sample(bots_word, 1))
                while True:
                    try:
                        try:
                            ...
                            if query.content in list2 and not (query.content in history):
                                ...
                                await ctx.send(bots_ans)
                            else:
                                if not (query.content in list2):
                                    await ctx.send("사전에 없는 단어입니다.")
                                else:
                                    await ctx.send("이미 사용된 단어입니다.")
                        except ValueError:
                            await ctx.send("봇이 단어를 찾지 못했습니다. 당신의 승리!")
                            break
                    except asyncio.TimeoutError:
                        await ctx.send("타임아웃! 봇의 승리")
                        break
            else:
                await ctx.send("게임이 종료되었습니다.")
        except asyncio.TimeoutError:
    ...
    @app_commands.command(name="블랙잭", description="퓨이린 봇과 블랙잭 게임을 합니다.")
        ...
        try:
            ...
            if res.content == '/시작':
                ...
                while True:
                    ...
                    if score_player == 21:
                        await ctx.send('블랙잭! 당신의 승리!')
                        await ctx.send('한 판 더? (y/n)')
                        def check19(res):
                        ...
                        try:
                            ...
                            await ctx.send('응답하지 않아 게임을 종료합니다.')
    ...
    @app_commands.command(name="업다운", description="퓨이린 봇과 업다운 게임을 합니다.")
    async def updown(self, interaction: discord.Interaction):
        ctx = await commands.Context.from_interaction(interaction)
        ...
        try:
            ...
            if res.content == '/시작':
                await ctx.send("업다운은 봇이 뽑은 숫자를 10회 안에 맞추는 게임입니다.")
                await ctx.send("숫자의 자릿수를 입력해주세요. 입력시간 초과시 기본값으로 설정됩니다. (기본값 : 3)")
                def check26(res):
                    ...
                    try:
                        ...
                        while count < 9 and int(res.content) != bots_num:
                            ...
                            try:
                                ...
                                await ctx.send("시간초과! 봇이 뽑은 숫자 : %d" %bots_num)
                        if int(res.content) == bots_num:
                            await ctx.send("축하합니다, 정답입니다! 봇이 뽑은 숫자 : %d" %bots_num)
                        else:
                            await ctx.send("봇이 뽑은 숫자는 [ %d ] 였습니다..." %bots_num)
                    except asyncio.TimeoutError:
                        await ctx.send("시간초과! 봇이 뽑은 숫자 : %d" %bots_num)
                except asyncio.TimeoutError:
                    await ctx.send("기본값으로 설정되었습니다.")
    ...
    @app_commands.command(name="랜덤게임", description="퓨이린 봇이 좋아하는 ~ 랜덤게임! 봇이 등록된 게임 중 무작위로 한가지를 불러옵니다.")
    async def randomgame(self, interaction: discord.Interaction):
        ctx = await commands.Context.from_interaction(interaction)
        game_list = ["가위바위보", "끝말잇기", "블랙잭", "업다운"]
        sample = random.sample(game_list, 1)
        await ctx.send("뽑힌 게임은 %s입니다." %sample)
        if sample[0] == "가위바위보":
            embed = discord.Embed(title="가위바위보!", description="", color=0x5CD1E5)
            embed.add_field(name="퓨이린의 랜덤게임" ,value="시작하려면 /시작\n끝내려면 /종료", inline=False)
            await ctx.send(embed=embed)
...
async def setup(bot):
    rg = RandomGame(bot)
    await bot.add_cog(rg)
    try:
        bot.tree.add_command(rg.rsp)
        bot.tree.add_command(rg.kkut)
        bot.tree.add_command(rg.blackjack)
        bot.tree.add_command(rg.updown)
        bot.tree.add_command(rg.randomgame)
