...
@app.command(name="골라", pass_context=True)
async def pick(ctx):
    ...
    try:
        message = await app.wait_for("message", timeout=20, check=check_p1)
        ...
        try:
            message = await app.wait_for("message", timeout=20, check=check_p2)
