            ...
            if sample[0] == "가위바위보":
                ...
                try:
                    ...
                    if res.content == '/시작':
                        ...
                        def check16(res2):
                            return message.author == res2.author and (res2.content == '가위' or res2.content == '바위' or res2.content == '보')
                        try:
                ...
                try:
                    ...
                    if res.content == '/시작':
                        ...
                        bots_word = list2
                        chosen = random.sample(bots_word, 1)
                        await message.channel.send("%s" %chosen)
                        while True:
                            try:
                                try:
                                    def check23(res):
                                        return message.author == res.author and len(res.content) >= 2 and res.content[0] == chosen[0][-1]
                                    query = await self.bot.wait_for("message", timeout=5.0, check=check23)
                                    ...
                                    if query.content in list2 and not (query.content in history):
                                        ...
                                        for word in list2:
                                            if word.startswith(start) and not (word in history):
                                                ...
                                                continue
                                        chosen = random.sample(ans_list, 1)
                ...
                try:
                    ...
                    if res.content == '/시작':
                        ...
                        def check26(res):
                            return res.content.isdigit() and message.author == res.author
                        try:
        ...
        if message.content == '/가위바위보':
            ...
            try:
                ...
                if res.content == '/시작':
                    await message.channel.send("가위, 바위, 보 중에 고르세요! 제한시간은 10초입니다!")
                    def check2(res2):
                        return message.author == res.author and (res2.content == '가위' or res2.content == '바위' or res2.content == '보')
                    try:
                    ...
                    while True:
                        try:
                            try:
                                def check29(res):
                                    return message.author == res.author and len(res.content) >= 2 and res.content[0] == chosen[0][-1]
                                query = await self.bot.wait_for("message", timeout=5.0, check=check29)
                                start = query.content[-1]
                                ans_list = []
                                if query.content in list2 and not (query.content in history):
                                    history.append(query.content)
